...
local function pick_latest_rockspec_version(name, versions)
   assert(type(name) == "string")
   assert(type(versions) == "table")
   ...
   if items then
      ...
      return path.make_url(items[pick].repo, name, version, "rockspec")
   end
   return nil
end
...
function find_suitable_rockspec(query)
   assert(type(query) == "table")
   ---[[
   local results, err = search_repos(query)
   if not results then
   ...
   if not first then
      return nil, "No results matching query were found."
   elseif not next(results, first) then
      return pick_latest_rockspec_version(query.name, results[first])
   else
      return results
   end
   --]]
