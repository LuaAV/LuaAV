...
extern const bool e2ga_metricEuclidean;
...
public:
	...
	/// Converts a I2i_t to a mv.
	inline mv(const I2i_t&A)  {set(A);}
	...
	/// Assignment operator (mv).
	inline mv &operator=(const I2i_t &A) {set(A); return *this;}
	/// Assignment operator (mv).
	...
	/// Sets this mv to the value of I2i_t A
	void set(const I2i_t &A);
	...
	void expand(const double *ptrs[3], bool nulls = true) const;
	...
	inline const char * c_str(const char *fp = NULL) const {
		...
		return ::e2ga::c_str(*this, buf, 2048, fp);
	}
...
public:
	inline double const *nullFloats() const {
		...
		return (nf == NULL) ? (nf = new double[4]) : nf;
	}
	...
	*/
	inline void reserveGroup_0() {
		if ((m_gu & 1) == 0) {
			...
			double *dst = m_c + e2ga_mvSize[newGroupUsageBelowNextGroup];
			double *src = m_c + e2ga_mvSize[groupUsageBelow];
			for (int i = e2ga_mvSize[groupUsageAbove]-1; i >= 0; i--) // work from end to start of array to avoid overwriting (dst is always beyond src)
				dst[i] = src[i];

			// set coordinates of new group to 0
			double *ptr = m_c + e2ga_mvSize[groupUsageBelow];
			ptr[0] = 0.0;
		}
	}
	/**
...
/// This class can hold a specialized multivector of type I2i_t.
/// 
/// The coordinates are stored in type double.
/// 
/// The type is constant.
...
public:
	...
	/// Floating point type used by I2i_t 
	typedef double Float;
	/// Array indices of I2i_t coordinates.
	typedef enum {
	} ArrayIndex;
	typedef enum {
		/// the order of coordinates (this is the type of the first argument of coordinate-handling functions)
	...
	/// Constructs a new I2i_t with variable coordinates set to 0.
	inline I2i_t() {set();}

	/// Copy constructor.
	inline I2i_t(const I2i_t &A) {set(A);}



	/// Constructs a new I2i_t from mv.
	/// \param A The value to copy. Coordinates that cannot be represented
	/// are silently dropped.
	/// \param filler This argument can have any value; it's role
	/// is only to prevent the compiler from using this constructor as a converter.
	inline I2i_t(mv &A, int filler) {set(A);}


	/// Assignment operator (I2i_t).
	inline I2i_t &operator=(const I2i_t &A) {if (this != &A) {set(A);} return *this;}
	
		

	/// Assignment operator (mv).
	inline I2i_t &operator=(const mv &A) {set(A); return *this;}
...
public:
	...
	inline vector(const CoordinateOrder co, const double A[2]) {set(co, A);}
	
	/// Constructs a new vector with each coordinate specified.
	inline vector(const CoordinateOrder co,  double e1, double e2) {
		set(co, e1, e2);
	}
...
public:
	...
	inline bivector(const CoordinateOrder co, const double A[1]) {set(co, A);}
	
	/// Constructs a new bivector with each coordinate specified.
	inline bivector(const CoordinateOrder co,  double e1_e2) {
		set(co, e1_e2);
	}
...
public:
	...
	inline rotor(const CoordinateOrder co, const double A[2]) {set(co, A);}
	
	/// Constructs a new rotor with each coordinate specified.
	inline rotor(const CoordinateOrder co,  double scalar, double e1_e2) {
		set(co, scalar, e1_e2);
	}
...
/// The coordinates are stored in type double.
/// 
/// There are 3 matrices, one for each grade.
...
public:
	...
	double m_m1[4];
	/// Matrix for grade 2; the size is 1 x 1
	double m_m2[1];
public:
	...
	inline om(const vector &ie1, const vector &ie2)
		{ set(ie1, ie2); }
	...
	void set(const vector &ie1, const vector &ie2);
...
public:
	...
	/// Constructor from images of basis vectors.
	inline grade1OM(const vector &ie1, const vector &ie2)
		{ set(ie1, ie2); }
...
public:
	...
	/// Constructor from images of basis vectors.
	inline grade2OM(const vector &ie1, const vector &ie2)
		{ set(ie1, ie2); }
...
/** rotor which rotates by 90 degrees */
...
/// Returns scalar part of  I2i_t
double _double(const I2i_t &x);
/// Returns scalar part of  I2i_t
inline double _Float(const I2i_t &x) {return _double(x); };
...
/// Returns I2_t + I2i_t.
double add(const I2_t &a, const I2i_t &b);
/// Returns mv - mv.
...
/// Returns e1_t - e2_t.
vector subtract(const e1_t &a, const e2_t &b);
/// Returns I2_t - I2i_t.
bivector subtract(const I2_t &a, const I2i_t &b);
...
/// Returns a * b * reverse(a) using default metric. Only gives the correct result when the versor has a positive squared norm.
/// 
vector applyVersorWI(const rotor &a, const e1_t &b, const rotor &c);
/// Returns a * b * inverse(a) using default metric.
bivector applyVersor(const rotor &a, const I2_t &b);
/// Returns a * b * reverse(a) using default metric. Only gives the correct result when the versor has a positive squared norm.
/// 
bivector applyUnitVersor(const rotor &a, const I2_t &b);
/// Returns a * b * reverse(a) using default metric. Only gives the correct result when the versor has a positive squared norm.
/// 
bivector applyVersorWI(const rotor &a, const I2_t &b, const rotor &c);
/// Returns a * b * inverse(a) using default metric.
...
/// Returns grade2OM * I2i_t.
bivector applyOM(const grade2OM &a, const I2i_t &b);
/// Returns a / b
...
/// Returns a / b
bivector div(const I2_t &a, const double b);
...
bivector dual(const double a);
/// Returns undual of double using default metric.
bivector undual(const double a);
/// Returns dual of vector using default metric.
vector dual(const vector &a);
/// Returns undual of vector using default metric.
vector undual(const vector &a);
/// Returns dual of bivector using default metric.
double dual(const bivector &a);
/// Returns undual of bivector using default metric.
double undual(const bivector &a);
/// Returns dual of rotor using default metric.
rotor dual(const rotor &a);
/// Returns undual of rotor using default metric.
rotor undual(const rotor &a);
/// Returns dual of I2_t using default metric.
double dual(const I2_t &a);
/// Returns undual of I2i_t using default metric.
double undual(const I2i_t &a);
/// Returns dual of e1_t using default metric.
vector dual(const e1_t &a);
/// Returns undual of e2_t using default metric.
vector undual(const e2_t &a);
...
/// Returns grade 1 of  I2_t.
double extractGrade1(const I2_t &a);
/// Returns grade 2 of  I2_t.
I2_t extractGrade2(const I2_t &a);
...
vector gp(const rotor &a, const vector &b);
/// Returns geometric product of vector and rotor.
vector gp(const vector &a, const rotor &b);
...
double gp(const bivector &a, const bivector &b);
/// Returns geometric product of e1_t and I2_t.
e2_t gp(const e1_t &a, const I2_t &b);
...
/// Returns a bitmap where each one-bit means that at least one coordinate of the respective grade of  a is larger than b
int gradeBitmap(const I2_t &a, const double b);
...
/// Returns Hadamard product (coordinate-wise multiplication) of e1_t and I2_t.
double hp(const e1_t &a, const I2_t &b);
...
/// Returns Hadamard product (coordinate-wise multiplication) of e1_t and e1_t.
e1_t ihp(const e1_t &a, const e1_t &b);
/// Returns Hadamard product (coordinate-wise multiplication) of e1_t and I2_t.
double ihp(const e1_t &a, const I2_t &b);
/// Returns Hadamard product (coordinate-wise multiplication) of vector and e1_t.
...
/// Returns scalar product of e1_t and vector.
double sp(const e1_t &a, const vector &b);
/// Returns left contraction of e2_t and vector.
double lc(const e2_t &a, const vector &b);
/// Returns right contraction of I2_t and bivector.
double rc(const I2_t &a, const bivector &b);
/// Returns Hestenes inner product of e1_t and bivector.
vector hip(const e1_t &a, const bivector &b);
/// Returns Modified Hestenes inner product of bivector and e1_t.
vector mhip(const bivector &a, const e1_t &b);
...
vector op(const vector &a, const rotor &b);
/// Returns outer product of e1_t and e2_t.
I2_t op(const e1_t &a, const e2_t &b);
/// Returns outer product of e1_t and I2_t.
double op(const e1_t &a, const I2_t &b);
...
/// exp of I2_t (uses fast special case)
rotor exp(const I2_t &a);
...
/// cosh of I2_t (uses fast special case)
double cosh(const I2_t &a);
...
/// sinh of I2_t (uses fast special case)
bivector sinh(const I2_t &a);
...
/// cos of I2_t (uses fast special case)
double cos(const I2_t &a);
...
/// sin of I2_t (uses fast special case)
bivector sin(const I2_t &a);
...
vector cliffordConjugate(const e1_t &a);
/// Returns grade involution of e2_t.
vector gradeInvolution(const e2_t &a);
/// Returns reverse of e2_t.
e2_t reverse(const e2_t &a);
/// Returns negation of I2_t.
bivector negate(const I2_t &a);
/// Returns Clifford conjugate of I2_t.
bivector cliffordConjugate(const I2_t &a);
/// Returns grade involution of I2i_t.
bivector gradeInvolution(const I2i_t &a);
/// Returns reverse of I2i_t.
I2_t reverse(const I2i_t &a);
...
/// Returns unit of I2_t using default metric.
I2_t unit(const I2_t &a);
/// Returns unit of I2i_t using default metric.
bivector unit(const I2i_t &a);
...
/// Returns versor inverse of a using default metric.
bivector versorInverse(const I2_t &a);
/// Returns versor inverse of a using default metric.
I2_t versorInverse(const I2i_t &a);
...
/// Returns true if all coordinates of a are abs <= b
bool zero(const I2_t &a, const double b);
/// returns add(a, b)
...
/// returns add(a, b)
inline double operator+(const I2_t &a, const I2i_t &b) {
	return add(a, b);
}
...
/// returns subtract(a, b)
inline bivector operator-(const I2_t &a, const I2i_t &b) {
	return subtract(a, b);
}
/// returns dual(a)
...
/// returns dual(a)
inline double operator*(const bivector &a) {
	return dual(a);
}
/// returns dual(a)
...
/// returns dual(a)
inline double operator*(const I2_t &a) {
	return dual(a);
}
/// returns dual(a)
...
/// returns gp(a, b)
inline vector operator*(const rotor &a, const vector &b) {
	return gp(a, b);
}
/// returns gp(a, b)
...
/// returns gp(a, b)
inline e2_t operator*(const e1_t &a, const I2_t &b) {
	return gp(a, b);
}
/// returns gp(a, b)
...
/// returns (a = increment(a))
inline rotor operator++(const I2_t &a) {
	return increment(a);
}
...
/// returns (a = decrement(a))
inline rotor operator--(const I2i_t &a) {
	return decrement(a);
}
/// returns sp(a, b)
inline mv operator%(const mv &a, const mv &b) {
...
/// returns sp(a, b)
inline double operator%(const e1_t &a, const vector &b) {
	return sp(a, b);
}
/// returns lc(a, b)
inline double operator<<(const e2_t &a, const vector &b) {
	return lc(a, b);
}
/// returns rc(a, b)
inline double operator>>(const I2_t &a, const bivector &b) {
	return rc(a, b);
}
/// returns sp(a, b)
...
/// returns op(a, b)
inline double operator^(const e1_t &a, const I2_t &b) {
	return op(a, b);
}
/// returns gp(a, b)
...
/// returns negate(a)
inline bivector operator-(const I2_t &a) {
	return negate(a);
}
/// returns reverse(a)
inline I2_t operator~(const I2i_t &a) {
	return reverse(a);
}
...
inline void I2i_t::set(const mv &src) {
	const double *ptr = src.getC();
...
inline void mv::set(const I2i_t &src) {
	setGroupUsage(4);
	double *ptr = m_c;
	ptr[0] = -1.0;
}
...
inline void vector::set(const CoordinateOrder co, const double _e1, const double _e2)
{
	m_e1 = _e1;
	m_e2 = _e2;

}
inline void bivector::set(const CoordinateOrder co, const double _e1_e2)
{
	m_e1_e2 = _e1_e2;

}
inline void rotor::set(const CoordinateOrder co, const double _scalar, const double _e1_e2)
{
	m_scalar = _scalar;
	m_e1_e2 = _e1_e2;
...
inline void I2i_t::set(const I2i_t &a)
{
...
inline double I2i_t::largestCoordinate() const {
	double maxValue = 1.0;
	return maxValue;
}
inline double I2i_t::largestBasisBlade(unsigned int &bm) const {
	double maxValue = 1.0;
	bm = 3;
	return maxValue;
}
...
inline double _double(const I2i_t &x) {
	return 0.0;
}
...
inline void om::setIdentity() {
	...
	m_m1[0] = m_m1[3] = m_m2[0] = 1.0;
}

inline void om::set(const om &src) {
	e2ga::copy_4(m_m1, src.m_m1);

	e2ga::copy_1(m_m2, src.m_m2);
...
inline void om::set(const grade2OM &src) {
	m_m2[0] =  src.m_m2[0];
	m_m1[0] = m_m1[1] = m_m1[2] = m_m1[3] = 0.0;
}
...
inline void grade1OM::set(const vector &ie1, const vector &ie2)
{
	// Set image of e1
	m_m1[0] = ie1.m_e1;
	m_m1[2] = ie1.m_e2;
...
inline void grade2OM::set(const vector &ie1, const vector &ie2)
{
	// Set image of e1^e2
...
{
	return bivector(bivector::coord_e1e2,
			R.m_e1_e2 // e1_e2
		);
...
{
	return vector(vector::coord_e1_e2,
			(a.m_e1+b.m_e1), // e1
			(a.m_e2+b.m_e2) // e2
		);
...
{
	return rotor(rotor::coord_scalar_e1e2,
			a.m_scalar, // scalar
			(a.m_e1_e2+b.m_e1_e2) // e1_e2
		);
...
inline double add(const I2_t &a, const I2i_t &b)
{
	return 0.0;
...
inline vector subtract(const e1_t &a, const e2_t &b)
{
	return vector(vector::coord_e1_e2,
			1.0, // e1
			-1.0 // e2
...
inline bivector subtract(const I2_t &a, const I2i_t &b)
{
	return bivector(bivector::coord_e1e2,
			2.0 // e1_e2
		);
...
{
	double _n2_ = (a.m_e1_e2*a.m_e1_e2+a.m_scalar*a.m_scalar);

	return vector(vector::coord_e1_e2,
			((-a.m_e1_e2*a.m_e1_e2+a.m_scalar*a.m_scalar))/(_n2_), // e1
			(-2.0*a.m_e1_e2*a.m_scalar)/(_n2_) // e2
		);
}
inline vector applyUnitVersor(const rotor &a, const e1_t &b)
{
	return vector(vector::coord_e1_e2,
			(-a.m_e1_e2*a.m_e1_e2+a.m_scalar*a.m_scalar), // e1
			-2.0*a.m_e1_e2*a.m_scalar // e2
		);
}
inline vector applyVersorWI(const rotor &a, const e1_t &b, const rotor &c)
{
	return vector(vector::coord_e1_e2,
			(a.m_e1_e2*c.m_e1_e2+a.m_scalar*c.m_scalar), // e1
			(-a.m_e1_e2*c.m_scalar+a.m_scalar*c.m_e1_e2) // e2
		);
}
inline bivector applyVersor(const rotor &a, const I2_t &b)
{
	double _n2_ = (a.m_e1_e2*a.m_e1_e2+a.m_scalar*a.m_scalar);

	return bivector(bivector::coord_e1e2,
			((a.m_e1_e2*a.m_e1_e2+a.m_scalar*a.m_scalar))/(_n2_) // e1_e2
		);
}
inline bivector applyUnitVersor(const rotor &a, const I2_t &b)
{
	return bivector(bivector::coord_e1e2,
			(a.m_e1_e2*a.m_e1_e2+a.m_scalar*a.m_scalar) // e1_e2
		);
}
inline bivector applyVersorWI(const rotor &a, const I2_t &b, const rotor &c)
{
	return bivector(bivector::coord_e1e2,
			(-a.m_e1_e2*c.m_e1_e2+a.m_scalar*c.m_scalar) // e1_e2
		);
}
inline vector applyVersor(const rotor &a, const vector &b)
{
	...
	return vector(vector::coord_e1_e2,
			((-a.m_e1_e2*a.m_e1_e2*b.m_e1+2.0*a.m_e1_e2*a.m_scalar*b.m_e2+a.m_scalar*a.m_scalar*b.m_e1))/(_n2_), // e1
			((-a.m_e1_e2*a.m_e1_e2*b.m_e2+-2.0*a.m_e1_e2*a.m_scalar*b.m_e1+a.m_scalar*a.m_scalar*b.m_e2))/(_n2_) // e2
		);
}
inline vector applyUnitVersor(const rotor &a, const vector &b)
{
	return vector(vector::coord_e1_e2,
			(-a.m_e1_e2*a.m_e1_e2*b.m_e1+2.0*a.m_e1_e2*a.m_scalar*b.m_e2+a.m_scalar*a.m_scalar*b.m_e1), // e1
			(-a.m_e1_e2*a.m_e1_e2*b.m_e2+-2.0*a.m_e1_e2*a.m_scalar*b.m_e1+a.m_scalar*a.m_scalar*b.m_e2) // e2
		);
}
inline vector applyVersorWI(const rotor &a, const vector &b, const rotor &c)
{
	return vector(vector::coord_e1_e2,
			(a.m_e1_e2*b.m_e1*c.m_e1_e2+a.m_e1_e2*b.m_e2*c.m_scalar+a.m_scalar*b.m_e1*c.m_scalar-a.m_scalar*b.m_e2*c.m_e1_e2), // e1
			(-a.m_e1_e2*b.m_e1*c.m_scalar+a.m_e1_e2*b.m_e2*c.m_e1_e2+a.m_scalar*b.m_e1*c.m_e1_e2+a.m_scalar*b.m_e2*c.m_scalar) // e2
		);
}
inline bivector applyVersor(const rotor &a, const bivector &b)
{
	...
	return bivector(bivector::coord_e1e2,
			((a.m_e1_e2*a.m_e1_e2*b.m_e1_e2+a.m_scalar*a.m_scalar*b.m_e1_e2))/(_n2_) // e1_e2
		);
}
inline bivector applyUnitVersor(const rotor &a, const bivector &b)
{
	return bivector(bivector::coord_e1e2,
			(a.m_e1_e2*a.m_e1_e2*b.m_e1_e2+a.m_scalar*a.m_scalar*b.m_e1_e2) // e1_e2
		);
}
inline bivector applyVersorWI(const rotor &a, const bivector &b, const rotor &c)
{
	return bivector(bivector::coord_e1e2,
			(-a.m_e1_e2*b.m_e1_e2*c.m_e1_e2+a.m_scalar*b.m_e1_e2*c.m_scalar) // e1_e2
		);
}
inline vector applyOM(const om &a, const vector &b)
{
	return vector(vector::coord_e1_e2,
			(a.m_m1[0]*b.m_e1+a.m_m1[1]*b.m_e2), // e1
			(a.m_m1[2]*b.m_e1+a.m_m1[3]*b.m_e2) // e2
		);
...
inline bivector applyOM(const grade2OM &a, const I2i_t &b)
{
	return bivector(bivector::coord_e1e2,
			-a.m_m2[0] // e1_e2
		);
...
inline bivector div(const I2_t &a, const double b)
{
	return bivector(bivector::coord_e1e2,
			1.0 / (b) // e1_e2
		);
}
inline bivector dual(const double a)
{
	return bivector(bivector::coord_e1e2,
			-a // e1_e2
		);
...
inline bivector undual(const double a)
{
	return bivector(bivector::coord_e1e2,
			a // e1_e2
		);
...
inline vector dual(const vector &a)
{
	return vector(vector::coord_e1_e2,
			a.m_e2, // e1
			-a.m_e1 // e2
...
inline vector undual(const vector &a)
{
	return vector(vector::coord_e1_e2,
			-a.m_e2, // e1
			a.m_e1 // e2
...
inline double dual(const bivector &a)
{
	return a.m_e1_e2;

}
inline double undual(const bivector &a)
{
	return -a.m_e1_e2;

}
inline rotor dual(const rotor &a)
{
	return rotor(rotor::coord_scalar_e1e2,
			a.m_e1_e2, // scalar
			-a.m_scalar // e1_e2
...
inline rotor undual(const rotor &a)
{
	return rotor(rotor::coord_scalar_e1e2,
			-a.m_e1_e2, // scalar
			a.m_scalar // e1_e2
...
inline double dual(const I2_t &a)
{
	return 1.0;

}
inline double undual(const I2i_t &a)
{
	return 1.0;

}
inline vector dual(const e1_t &a)
{
	return vector(vector::coord_e1_e2,
			0.0, // e1
			-1.0 // e2
...
inline vector undual(const e2_t &a)
{
	return vector(vector::coord_e1_e2,
			-1.0, // e1
			0.0 // e2
...
inline double extractGrade1(const I2_t &a)
{
	return 0.0;
}
inline I2_t extractGrade2(const I2_t &a)
{
	return I2_t(		);
}
...
inline vector gp(const rotor &a, const vector &b)
{
	return vector(vector::coord_e1_e2,
...
inline vector gp(const vector &a, const rotor &b)
{
	return vector(vector::coord_e1_e2,
...
inline double gp(const bivector &a, const bivector &b)
{
	return -a.m_e1_e2*b.m_e1_e2;

}
inline e2_t gp(const e1_t &a, const I2_t &b)
{
	return e2_t(		);
...
inline int gradeBitmap(const I2_t &a, const double b)
{
	int bitmap = 0;
	if (1.0 > b) bitmap |= 4;
	return bitmap;
...
inline double hp(const e1_t &a, const I2_t &b)
{
	return 0.0;
...
inline e1_t ihp(const e1_t &a, const e1_t &b)
{
	return e1_t(		);

}
inline double ihp(const e1_t &a, const I2_t &b)
{
	return 0.0;
...
inline rotor increment(const I2_t &a)
{
	return rotor(rotor::coord_scalar_e1e2,
			1.0, // scalar
			1.0 // e1_e2
...
inline rotor decrement(const I2i_t &a)
{
	return rotor(rotor::coord_scalar_e1e2,
			-1.0, // scalar
			-1.0 // e1_e2
...
inline double sp(const e1_t &a, const vector &b)
{
	return b.m_e1;

}
inline double lc(const e2_t &a, const vector &b)
{
	return b.m_e2;

}
inline double rc(const I2_t &a, const bivector &b)
{
	return -b.m_e1_e2;

}
inline vector hip(const e1_t &a, const bivector &b)
{
	return vector(vector::coord_e1_e2,
			0.0, // e1
			b.m_e1_e2 // e2
...
inline vector mhip(const bivector &a, const e1_t &b)
{
	return vector(vector::coord_e1_e2,
			0.0, // e1
			-a.m_e1_e2 // e2
...
{
	return ::fabs(::sqrt((a.m_e1*a.m_e1+a.m_e2*a.m_e2)));
...
{
	return ::fabs(::sqrt(a.m_e1_e2*a.m_e1_e2));
...
{
	return ::fabs(::sqrt((a.m_e1_e2*a.m_e1_e2+a.m_scalar*a.m_scalar)));
...
{
	return (a.m_e1*a.m_e1+a.m_e2*a.m_e2);
...
{
	return (a.m_e1_e2*a.m_e1_e2+a.m_scalar*a.m_scalar);
...
inline vector op(const vector &a, const rotor &b)
{
	return vector(vector::coord_e1_e2,
			a.m_e1*b.m_scalar, // e1
			a.m_e2*b.m_scalar // e2
...
inline I2_t op(const e1_t &a, const e2_t &b)
{
	return I2_t(		);

}
inline double op(const e1_t &a, const I2_t &b)
{
	return 0.0;
...
{
	double _alpha = ::sqrt(::fabs(-a.m_e1_e2*a.m_e1_e2));
...
inline rotor exp(const I2_t &a)
{
	double _alpha = ::sqrt(::fabs(-1.0));
...
inline double cosh(const I2_t &a)
{
	double _alpha = ::sqrt(::fabs(-1.0));
...
inline bivector sinh(const I2_t &a)
{
	double _alpha = ::sqrt(::fabs(-1.0));
...
inline double cos(const I2_t &a)
{
	double _alpha = ::sqrt(::fabs(-1.0));
...
inline bivector sin(const I2_t &a)
{
	double _alpha = ::sqrt(::fabs(-1.0));
...
inline vector cliffordConjugate(const e1_t &a)
{
	return vector(vector::coord_e1_e2,
			-1.0, // e1
			0.0 // e2
...
inline e2_t reverse(const e2_t &a)
{
	return e2_t(		);

}
inline bivector negate(const I2_t &a)
{
	return bivector(bivector::coord_e1e2,
			-1.0 // e1_e2
		);
...
inline bivector cliffordConjugate(const I2_t &a)
{
	return bivector(bivector::coord_e1e2,
			-1.0 // e1_e2
		);
...
inline bivector gradeInvolution(const I2i_t &a)
{
	return bivector(bivector::coord_e1e2,
			-1.0 // e1_e2
		);
...
inline I2_t reverse(const I2i_t &a)
{
	return I2_t(		);
...
{
	double _n_ = ::sqrt((a.m_e1*a.m_e1+a.m_e2*a.m_e2));
...
{
	double _n_ = ::sqrt(a.m_e1_e2*a.m_e1_e2);
...
{
	double _n_ = ::sqrt((a.m_e1_e2*a.m_e1_e2+a.m_scalar*a.m_scalar));
...
inline I2_t unit(const I2_t &a)
{
	return I2_t(		);
}
inline bivector unit(const I2i_t &a)
{
	return bivector(bivector::coord_e1e2,
			-1.0 // e1_e2
		);
...
{
	double _n2_ = (a.m_e1*a.m_e1+a.m_e2*a.m_e2);
...
{
	double _n2_ = a.m_e1_e2*a.m_e1_e2;
...
inline bivector versorInverse(const I2_t &a)
{
	return bivector(bivector::coord_e1e2,
			-1.0 // e1_e2
		);
}
inline I2_t versorInverse(const I2i_t &a)
{
	return I2_t(		);
}
...
inline bool zero(const I2_t &a, const double b)
{
	if (1.0 > b) return false;
	return true;
}
...
*/
bool parseEx(struct e2gaParseMultivectorData &data, const std::string &str, const std::string &strSourceName);
