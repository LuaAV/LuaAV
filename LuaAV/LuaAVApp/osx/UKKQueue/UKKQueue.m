...
-(void) addPathToQueue: (NSString*)path notifyingAbout: (u_int)fflags
{
    ...
    if( fd >= 0 )
    {
//		NSLog(@"addPathToQueue: %@ %d", path, fd);
...
//        NSLog(@"removePathFromQueue: %@ %d", path, fd);
...
-(BOOL) watchingFile:(NSString *)file
{
	return [watchedPaths containsObject:file];
}
...
-(void)		watcherThread: (id)sender
{
    ...
    while( keepThreadRunning )
    {
		...
		//				NSLog(@"UKKQueue: Detected file change: %@ %d", fpath, [[watchedFDs objectAtIndex: [watchedPaths indexOfObject: fpath]] intValue]);
