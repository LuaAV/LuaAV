...
-- copied from Dimiter Malkia's ufo project

local libs = {
   OSX     = { x86 = "lib/OSX/sdl.dylib", x64 = "lib/OSX/sdl.dylib" },
   Windows = { x86 = "lib/Windows/x86/SDL.dll", x64 = "lib/Windows/x64/SDL.dll" },
   Linux   = { x86 = "SDL", x64 = "lib/Linux/x64/libSDL.so", arm = "lib/Linux/arm/libSDL.so" },
}

local libname  = libs[ ffi.os ][ ffi.arch ]
...
return setmetatable(m, {
	__index = function(self, k)
		local ok, r = pcall(resolve, "SDL_", k)
		if not ok then 
			r = lib[k]
		end
		self[k] = r
