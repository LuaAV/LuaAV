...
static void c2gaParseWedgeBasisVector(unsigned int bvIdx, double *scale, unsigned int *bitmap) {
	unsigned b;
...
static void c2gaParseSum(double coord[16], double scale, unsigned int bitmap) {
	int idx = c2ga_basisElementIndexByBitmap[bitmap];
	coord[idx] += scale / (double)c2ga_basisElementSignByIndex[idx];
}

static void c2gaParseDataInit(struct c2gaParseMultivectorData *PD) {
	PD->message[0] = 0;
	PD->value.set();
}

/** returns true when 'buf' starts with 'pattern' */
static int c2gaCompareId(const char *pattern, const char *buf, int len) {
	int i = 0;
	while (pattern[i] != 0) {
...
*/
int c2gaParseGetNextToken(const char *buf, int *startIdx, int *endIdx, int *lineIdx, int *currentLineStart) {
	int i, pointFound = 0;
	...
	if (buf[(*startIdx)] == '^') {*endIdx = *startIdx; return T_WEDGE;} /*   ^   */
		...
		*endIdx = (*startIdx) + 1;
		...
		for (i = 0; i < 4; i++)
			if (c2gaCompareId(c2ga_basisVectorNames[i], buf + (*startIdx), (*endIdx) - (*startIdx) + 1)) return T_FIRST_BASIS_VECTOR + i; /* basis vector */
		return T_BAD_IDENTIFIER; /* bad identifier */
	}

	else return T_BAD_CHARACTER;
} /* end of c2gaParseGetNextToken */

bool parseEx(struct c2gaParseMultivectorData *PD, const std::string &_str, const std::string &_strSourceName)
{
	double coord[16];
	...
	c2gaParseDataInit(PD); /* should reset all */
	c2ga::zero_16(coord);

	/* get the first token */
	token = c2gaParseGetNextToken(str, &startIdx, &endIdx, &lineIdx, &currentLineStart);
	...
	while (1) {
		...
		if ((token == T_NUMBER) || ((token >= T_FIRST_BASIS_VECTOR) && (token <= T_LAST_BASIS_VECTOR))) { /* must be number or basis vector */
			...
			if (!beDone) {
				if ((token >= T_FIRST_BASIS_VECTOR) && (token <= T_LAST_BASIS_VECTOR)) {
					c2gaParseWedgeBasisVector((unsigned int)token - T_FIRST_BASIS_VECTOR, &scale, &bitmap);
					startIdx = endIdx+1;
				}
				else {
		...
		/* add */
		c2gaParseSum(coord, scale, bitmap);
...
{
	struct c2gaParseMultivectorData PD;
