...
module('renderfx.motionblur2', setconstructor)
----------------------------------------------------------
-- @param
...
function C:__call(init)
	...
	local oslab = Slab{
		ctx = init.ctx,
		file = findfile("cf.motionblur.vec.shl"),
	}
...
function M:draw(scene, vel)
	...
	--oslab:draw(scene, vel)
	---[[
	slabs[1]:draw(scene, vel)
	slabs[2]:draw(slabs[1], vel)
	slabs[3]:draw(slabs[2], vel)
	slabs[4]:draw(slabs[3], vel)
	oslab:draw(slabs[4], vel)
	--]]
end